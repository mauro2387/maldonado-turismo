 ...
 *
 * Era de 24 horas, que es lo que se querría: la velocidad de una línea cambia
 * con la temporada y un día es lo más parecido a "cómo anda hoy". Pero un día
 * sólo alcanza si el poller viene corriendo sin parar, y en invierno ni así:
 * medido contra la base, 24 horas dejaban **un** cuadro de medio kilómetro por
 * encima del mínimo de muestras, y todo lo demás caía a la constante de 18
 * km/h. Aplicada a la ruta, esa constante hacía que la app calculara 46% más
 * minutos que los que publican las empresas.
 *
 * Con siete días son 682 cuadros y 62 de 68 recorridos medidos, y el contraste
 * contra el horario publicado -que es la fuente independiente- pasa de 1,455 a
 * 1,032. Se puede verificar en `GET /transport/health/speeds`.
 *
 * El costo de la ventana larga es mezclar una semana de tránsito distinto. Es
 * un costo chico al lado de no medir nada: lo que se pierde es la diferencia
 * entre un martes y un sábado, y lo que se gana es la diferencia entre la ruta
 * y el centro.
 */
